...
//!
//! Bridges raw device enumeration ([`molto2_hid`]) and the friendly-name
//! registry ([`molto2_keyring`]) into one place, so the CLI (`moltoctl`) and the
//! GUI (`moltoui`) are thin front-ends over a single resolver rather than each
...
use molto2_keyring::{ConnectedKey, IdSource};
...
pub fn read_effective_serial(d: &HidDevice) -> Result<(String, IdSource), String> {
    ...
    if d.vendor_id == VID_YUBICO {
        let readers = molto2_transport::yubikey_ccid_serials().map_err(|e| e.to_string())?;
...
mod tests {
    use super::*;
    use molto2_hid::{HID_USAGE_FIDO_AUTHENTICATOR, HID_USAGE_PAGE_FIDO};
