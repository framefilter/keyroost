...
impl App {
    ...
    pub(crate) fn load_otp_entries(&mut self) {
        ...
        self.spawn_job("Reading OTP entries\u{2026}", move || {
            let result =
                (|| -> Result<(Vec<OtpRow>, &'static str, Option<String>, Option<bool>, Option<&'static str>, Option<IfaceState>), OtpTransportError> {
                    ...
                    let rows = entries
                        ...
                        .collect();
                    Ok((rows, active, serial, touch_ok, touch_why, iface))
                })();
            Box::new(move |app: &mut App| {
                ...
                match result {
                    Ok((rows, active, serial, touch_ok, touch_why, iface)) => {
                        ...
                        app.otp.touch_hotp_ok = touch_ok;
                        app.otp.touch_hotp_why = touch_why;
                        app.otp.iface = iface;
                        app.otp.error = None;
                    }
                    Err(e) => {
