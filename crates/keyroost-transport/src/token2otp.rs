...
impl Token2OtpSession {
    ...
    /// Read the serial number (spec §6.10). Not in the vendor manual — the FIDO
    /// applet answers it. Over PC/SC a FIDO-applet SELECT is sent first.
    pub fn read_serial(&mut self) -> Result<Vec<u8>, OtpTransportError> {
        if self.is_pcsc {
            // PC/SC must SELECT the FIDO applet before GET_INFO (spec §5, §6.10).
            let (_, sw) = self
                .transport
