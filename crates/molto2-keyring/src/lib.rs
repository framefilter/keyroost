    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ResolveError::UnknownName { name, known } if known.is_empty() => write!(
                f,
                "no key named '{}': no named keys yet — add one with `moltoctl key-name add`",
                name
            ),
...
/// Default config path: `$XDG_CONFIG_HOME/moltoui/keys.json`, else
/// `$HOME/.config/moltoui/keys.json`.
pub fn config_path() -> Option<PathBuf> {
    if let Some(xdg) = std::env::var_os("XDG_CONFIG_HOME") {
        if !xdg.is_empty() {
            return Some(PathBuf::from(xdg).join("moltoui").join("keys.json"));
        }
    }
    ...
    Some(PathBuf::from(home).join(".config").join("moltoui").join("keys.json"))
}
...
mod tests {
    ...
    fn load_missing_is_empty() {
        let k = Keyring::load_from(Path::new("/nonexistent/moltoui/keys.json")).unwrap();
