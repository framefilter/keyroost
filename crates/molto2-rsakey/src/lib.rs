...
//!
//! A small front-end helper shared by `moltoctl` and `moltoui`. It owns the one
...
//! (`e, p, q, u, dp, dq, n`, minimal big-endian) that `molto2_openpgp`'s import
...
/// without the modulus). Borrow these as `molto2_openpgp::RsaPrivateKeyParts`.
